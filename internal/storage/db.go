...
func createDB(DBConn string, logger logger.Logger) (*pgx.Conn, error) {
	...
	if err != nil {
		logger.Error("Error while create accural migration", slog.String("error", err.Error()))
		return nil, err
	}
	if err = m.Up(); err != nil && !errors.Is(err, migrate.ErrNoChange) {
		logger.Error("Error while accural migration up", slog.String("error", err.Error()))
		return nil, err
	}
	logger.Info("Migration accural complete!")
